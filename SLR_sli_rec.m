...
%%%% get
AllPrdBase = 1./( 1 + exp( -( TestInput * x_weight + c_intercept ) ) );
AllPrdBase =  mat2cell(  AllPrdBase,   repmat(1,size(AllPrdBase,1),1) , 1  );
AllPrd = cellfun( @(Mat) 1./( 1 + exp( -(Mat * x_weight + c_intercept) ) ),  All_test,'UniformOutput', false);
AllPrdThresh = cellfun( @( Mat, Thresh ) Mat > Thresh , AllPrd, AllPrdBase,'UniformOutput', false);
...
AllPrdThreshReOrder = cellfun( @(Vec,Idx) Vec(Idx), AllPrdThresh, AllPrd_Idx,'UniformOutput', false);
LogRec = cellfun( @(Vec,Idx) Vec(Idx), TotalTestIdx, AllPrd_Idx,'UniformOutput', false);
LogRec = cellfun( @(Vec,Idx) Vec( find( Idx==1 )  ), LogRec, AllPrdThreshReOrder,'UniformOutput', false);

LogRec = cellfun( @(Vec,Vec2) setdiff(Vec,Vec2,'stable'), LogRec, RecNonZero,'UniformOutput', false);
LogRecNum = cellfun( @(Mat) numel( Mat ), LogRec ,'UniformOutput', false);
LogRecNum = cellfun( @(Num,  Num2) ( (RecTopN-Num) >= Num2)*Num2 + ( (RecTopN-Num) < Num2)*(RecTopN-Num), RecNonZeroSize, LogRecNum,'UniformOutput', false);
LogRec = cellfun( @(Vec,Num) Vec(1:Num).', LogRec, LogRecNum,'UniformOutput', false);

MethodLogR = cellfun( @(Vec,Vec2) [Vec Vec2] , RecNonZero, LogRec,'UniformOutput', false);

MethodLogRLeftNum = cellfun( @(Mat) size(Mat, 2), MethodLogR ,'UniformOutput', false);
MethodLogRLeftLessIdx = cellfun( @(Vec) Vec < RecTopN , MethodLogRLeftNum ,'UniformOutput', false);
MethodLogRLeftNum = cellfun( @(Num, Idx) (Idx==true)*(RecTopN - Num)+(Idx==false) * 0, MethodLogRLeftNum, MethodLogRLeftLessIdx, 'UniformOutput', false);
IndirecGet = cellfun( @(Vec,Num) Vec(1:Num), Indirec,MethodLogRLeftNum ,'UniformOutput', false);

% Combine
MethodLogR = cellfun( @(Vec,Vec2) [Vec Vec2] , MethodLogR, IndirecGet,'UniformOutput', false);
...
LogRecNum = cellfun( @(Mat) numel( Mat ), LogRec ,'UniformOutput', false);
LogRecNum = cellfun( @(Num,  Num2) ( (RecTopN-Num) >= Num2)*Num2 + ( (RecTopN-Num) < Num2)*(RecTopN-Num), RecNonZeroSize, LogRecNum,'UniformOutput', false);
%LogRec = cellfun( @(Vec,Num) Vec(1:RecTopN-Num).', LogRec, RecNonZeraoSize,'UniformOutput', false);
LogRec = cellfun( @(Vec,Num) Vec(1:Num).', LogRec, LogRecNum,'UniformOutput', false);
