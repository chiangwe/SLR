function [TestRectTopNMatrix,TestRectTopNMatrixValue,TestIndex,RecScrSortIndexTopN] = RecMatrixSide(Test,Wcoef,seed,topN)
%index
...
RuleOut = ( -ones(size(RecScr)) ) .* (Test == 1) + ( ones(size(RecScr)) ) .* (Test == 0);
